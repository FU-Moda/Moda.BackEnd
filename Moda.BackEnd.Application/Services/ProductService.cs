        ...
        public async Task<AppActionResult> GetAllProduct(int pageNumber, int pageSize)
        {
            ...
            try
            {
                ...
                if (shopPackagesDb.Items != null && shopPackagesDb.Items.Count > 0)
                {
                    ...
                    {
                        var productListConfirm = await _productRepository.GetAllDataByExpression(p => p.Status == ProductStatus.CONFIRMED && p.ShopId == shop.ShopId, pageNumber, pageSize, null, false, p => p.Shop!, p => p.Shop!);
                        if (productListConfirm!.Items!.Count > 0 && productListConfirm.Items != null)
                        {
                            foreach (var item in productListConfirm.Items)
                            {
                            ...
                            result.Result = new PagedResult<ProductResponse>
                            {
                                ...
                                TotalPages = productListConfirm.TotalPages
                            };
                        }
                    }
                }
        ...
        public async Task<AppActionResult> GetProductByFilter(ProductFilter productFilter, int pageNumber, int pageSize)
        {
            ...
            try
            {
                ...
                if (shopPackagesDb.Items != null && shopPackagesDb.Items.Count > 0)
                {
                    ...
                    foreach (var shop in sortedShopPackages)
                    {
                        var productListConfirm = await _productRepository.GetAllDataByExpression(p => (p.Status == ProductStatus.CONFIRMED && p.ShopId == shop.ShopId) || p.Gender == productFilter.Gender || p.ClothType == productFilter.ClothType, pageNumber, pageSize, null, false, p => p.Shop!, p => p.Shop!);
                        if (productListConfirm.Items != null && productListConfirm.Items.Count > 0)
                        {
                            foreach (var item in productListConfirm.Items)
                            {
                                var productResponse = new ProductResponse();
